...
class AppStore extends Component {
  ...
  onFilteredCategory = (searchResults) => {
  ...
  onGetSearchResultApp = () => {
    const {inputValue} = this.state
    const inputBasisChange = appsList.filter(eachItem => eachItem.appName.toLowerCase().includes(inputValue.toLowerCase()))

    return inputBasisChange

  }

  render() {
    const {activeTabId,inputValue} = this.state
    const searchResults = this.onGetSearchResultApp()
    const filteredApps = this.onFilteredCategory(searchResults)
    return (
        ...
        <div className="content-container">
          ...
          <ul className="app-unordered-list">
            {filteredApps.map(eachAppItem => (
              <AppItem eachApp={eachAppItem} key={eachAppItem.appId} />
            ))}
          </ul>
          <h1>{activeTabId}</h1>
          <h1>{inputValue}</h1>
